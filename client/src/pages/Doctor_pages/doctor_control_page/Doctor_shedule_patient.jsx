...
export default function Doctor_shedule_patient() {
  ...
  // Handle input changes in reply form
  ...
  // Filter and search appointments using useMemo for performance
  const filteredAppointments = useMemo(() => {
    if (!appointments) return [];

    // Filter completed appointments
    const completedAppointments = appointments.filter(
      (token) => token.status === 'shedule'
    );

    if (!searchTerm.trim()) return completedAppointments;

    const term = searchTerm.toLowerCase();

    return completedAppointments.filter((token) => {
      const patientName = token?.patientId?.name?.toLowerCase() || '';
      const patientPhone = token?.patientId?.phone?.toLowerCase() || '';
  ...
  return (
    <div className="md:p-2 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-blue-900 mb-4">
        🧑 Completed Patients for Dr. {user?.user?.name}
      </h1>
